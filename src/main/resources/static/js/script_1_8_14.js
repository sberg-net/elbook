...
function vzdEintragUebersicht(showCache,makeCSVExport) {
  ...
  if ($('#vzdSearchForm #uid').length > 0) {
    initalLoading = false;
    Object.keys(elbookContext.searchData).forEach(function(key,index) {
      if (elbookContext.arraySearchData[key]) {
        elbookContext.searchData[key] = $('#' + key).val().trim() !== ''?$('#' + key).val().split(','):[];
      }
      else {
        elbookContext.searchData[key] = $('#' + key).val().trim();
      }

      if (elbookContext.searchData[key] === '') {
        elbookContext.searchData[key] = null;
      }
    ...
    Object.keys(elbookContext.searchData).forEach(function(key,index) {
      if (elbookContext.checkSearchData[key]) {
        if (Array.isArray(elbookContext.searchData[key]) && elbookContext.searchData[key].length > 0) {
          validSearch = true;
        }
        else if (!Array.isArray(elbookContext.searchData[key]) && elbookContext.searchData[key] !== null) {
          validSearch = true;
        }
      }
    });
    ...
    if (!validSearch &&
      (
        elbookContext.searchData['stateOrProvinceName'] !== 'UNKNOWN' || elbookContext.searchData['entryType'] !== 'UNKNOWN' ||
        elbookContext.searchData['dataFromAuthority'] !== 'UNDEFINED' || elbookContext.searchData['personalEntry'] !== 'UNDEFINED' ||
        elbookContext.searchData['active'] !== 'UNDEFINED'
      )) {
      validSearch = true;
    }
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      Object.keys(elbookContext.searchData).forEach(function(key,index) {
        if (elbookContext.arraySearchData[key]) {
          if (elbookContext.searchData[key] && elbookContext.searchData[key].length > 0) {
            $('#' + key).val(elbookContext.searchData[key].join(','));
          }
        }
        else {
          $('#'+key).val(elbookContext.searchData[key]);
        }
      });
