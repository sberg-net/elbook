...
public class VerzeichnisdienstService {
    ...
    public DirectoryEntry ladeByTelematikId(Mandant mandant, String telematikId) throws Exception {
    ...
    public DirectoryEntry ladeByUid(Mandant mandant, String uid) throws Exception {
        ReadDirEntryCommand readDirEntryCommand = new ReadDirEntryCommand();
    ...
    private DirectoryEntry lade(Mandant mandant, ReadDirEntryCommand readDirEntryCommand) throws Exception {
        ...
        List<DirectoryEntry> entries = defaultCommandResultCallbackHandler.getDirectoryEntries(readDirEntryCommand.getId());
    ...
    public String createCSVExport(Mandant mandant, List<DirectoryEntryContainer> searchResults) throws Exception {
        ...
        for (DirectoryEntryContainer directoryEntryContainer : searchResults) {
            DirectoryEntry directoryEntry = directoryEntryContainer.getDirectoryEntry();
            csvLines.add(new String[]{directoryEntry.getDirectoryEntryBase().getDn().getUid(),
                    directoryEntry.getDirectoryEntryBase().getTelematikID()});
        }
    ...
    @SuppressWarnings("unchecked")
    public List<UserCertificate> ladeZertifikate(TiVZDProperties tiVZDProperties, String uid) throws Exception {
        ...
        List<UserCertificate> entries = commandResultCallbackHandler.getUserCertificates(readDirCertCommand.getId());
    ...
    @SuppressWarnings("unchecked")
    public UserCertificate ladeZertifikat(TiVZDProperties tiVZDProperties, String uid, String certUid) throws Exception {
    ...
    public void loeschen(Mandant mandant, TiVZDProperties tiVZDProperties, String uid) throws Exception {
        DirectoryEntry directoryEntry = ladeByUid(mandant, uid);
        DirectoryEntryContainer directorEntryContainer = new DirectoryEntryContainer(directoryEntry, null, mandant);
        if (directoryEntry == null) {
            throw new IllegalStateException("Eintrag mit der uid: " + uid + " ist nicht vorhanden");
        } else if (directorEntryContainer.getKimMails() != null && !directorEntryContainer.getKimMails().isEmpty()) {
            throw new IllegalStateException("Eintrag mit der uid: " + uid + " kann nicht gelöscht werden, da KIM-Mails zugeordnet sind");
        } else if (!directoryEntry.getDirectoryEntryBase().getHolder().isEmpty()
                && !directoryEntry.getDirectoryEntryBase().getHolder().contains(tiVZDProperties.getAuthId())) {
    ...
    @SuppressWarnings("unchecked")
    public List<LogEntry> logEntries(TiVZDProperties tiVZDProperties, String uid) throws Exception {
        ...
        List<LogEntry> entries = commandResultCallbackHandler.getLogEntries(readDirLogEntryCommand.getId());
        if ((entries == null || entries.isEmpty()) && !commandResultCallbackHandler.getExceptions(readDirLogEntryCommand.getId()).isEmpty()) {
            ...
            if (!commandResultCallbackHandler.getResultReasons(readDirLogEntryCommand.getId()).get(0).getResultReason()
                    .equals(AbstractCommandResultCallbackHandler.ResultReason.NO_SEARCH_RESULTS)) {
                throw new IllegalStateException("entry logs not found for the id: " + uid);
            } else {
                entries = new ArrayList<>();
            }
        }
    ...
    public DefaultCommandResultCallbackHandler uebersicht
            (TiVZDProperties tiVZDProperties, ReadDirSyncEntryCommand readDirSyncEntryCommand) throws Exception {
        readDirSyncEntryCommand.setBaseEntryOnly(true);
        return tiVZDConnector.executeCommand(readDirSyncEntryCommand, tiVZDProperties);
    }
    ...
    public void pruefeZertifikate(
        ...
        for (UserCertificate userCertificate : vzdCerts) {
            if (userCertificate.getUserCertificate() == null) {
                continue;
            }
            VerzeichnisdienstZertifikat verzeichnisdienstZertifikat = erstelle(userCertificate);
        ...
        for (String newCert : newCerts) {
            ...
            if (!vzdCertMap.containsKey(Objects.requireNonNull(verzeichnisdienstZertifikat).getSerienNummer())) {
                ...
                try {
                    ...
                    DistinguishedName distinguishedNameCert = (DistinguishedName) dnNameCertL.get(0);
                    verzeichnisdienstImportErgebnis.getLog().add("zertifikat erfolgreich gespeichert: telematikid = "
                            + telematikId + ",uid = " + uid + ", certUid= " + distinguishedNameCert.getCn());
                    logEintragService.handle(mandant, telematikId, verwaltungsId, uid, distinguishedNameCert.getCn(), artikelTyp);
                    vzdCertMap.put(verzeichnisdienstZertifikat.getSerienNummer(), verzeichnisdienstZertifikat);
                } catch (Exception e) {
    ...
    public VerzeichnisdienstZertifikat erstelle(UserCertificate vzdCertificate) throws Exception {
        return erstelle(vzdCertificate.getUserCertificate(), vzdCertificate.getTelematikID());
    }
}
