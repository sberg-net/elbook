...
import de.gematik.vzd.api.DirectoryEntrySynchronizationApi;
import de.gematik.vzd.model.DirectoryEntry;
import de.gematik.vzd.model.ReadDirectoryEntryforSyncResponse;
...
public class ReadDirSyncEntryCommandHandler extends AbstractCommandHandler {
    ...
    public boolean checkValidation() {
        ...
        if (!check) {
            log.error("Missing arguments fpr read: "+command.command());
        }
        ...
        try {
            ...
            while (true) {
                ...
                if (pagingInfo == null || pagingInfo.getPagingCookie() == null) {
                    response = directoryEntrySynchronizationApi.readDirectoryEntryForSyncWithHttpInfo(uid, givenName, sn, cn,
                    ...
                    response = directoryEntrySynchronizationApi.readDirectoryEntryForSyncPagingWithHttpInfo(uid, givenName, sn, cn,
                            displayName, streetAddress, postalCode, countryCode, localityName, stateOrProvinceName, title,
                            organization, otherName, telematikID, telematikIDSubstr, specialization, domainID, holder, personalEntry,
                ...
                if (response.getStatusCode() == HttpStatus.OK) {
                    ...
                    if (directoryEntries.isEmpty()) {
                        ...
                        for (Iterator<DirectoryEntry> iterator = directoryEntries.iterator(); iterator.hasNext(); ) {
                            ...
                            commandResultCallbackHandler.handleDirectoryEntry(command, directoryEntry);
                            size++;
