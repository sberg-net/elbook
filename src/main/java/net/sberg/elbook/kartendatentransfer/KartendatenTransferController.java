...
public class KartendatenTransferController extends AbstractWebController {
    ...
    public String hbakartendatentransferSpeichern(Authentication authentication, @RequestBody KartendatenTransfer kartendatenTransfer) throws Exception {
        ...
        if (kartendatenTransfer.getId() == 0) {
            ...
            dbKartendatenTransfer.setAktivierungsCode(kartendatenTransferService.createAktivierungsCode());
    ...
    public String hbakartendatentransferUebersicht(Authentication authentication, Model model, String searchValue) throws Exception {
        ...
        if (searchValue.equals("")) {
            daten = genericDao.selectMany(
                    "select id,count,aktivierungsCode,empfaengerKartenherausgeber,erstelltAm,gelesenAm,versendetAm,gueltigBis,(gueltigBis < CURRENT_TIMESTAMP) abgelaufen from KartendatenTransfer where mandantId = ?",
            ...
            daten = genericDao.selectMany(
                    "select id,count,aktivierungsCode,empfaengerKartenherausgeber,erstelltAm,gelesenAm,versendetAm,gueltigBis,(gueltigBis < CURRENT_TIMESTAMP) abgelaufen from KartendatenTransfer where mandantId = ? and (aktivierungsCode like ? or hashCode like ?)",
                    KartendatenTransferReduziert.class.getName(),
                    null,
                    Arrays.asList(
                            new DaoPlaceholderProperty("mandantId", authUserDetails.getMandant().getId()),
