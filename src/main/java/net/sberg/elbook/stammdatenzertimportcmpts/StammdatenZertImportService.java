            ...
            try {
                ...
                if (verzeichnisdienstImportCommand.toDelete(vzdObject)) {
                    ...
                    if (vzdObject == null) {
                        ...
                        verzeichnisdienstService.loeschen(mandant, tiVZDProperties, vzdObject.getDirectoryEntryBase().getDn().getUid());

                        verzeichnisdienstImportErgebnis.setVzdUid(vzdObject.getDirectoryEntryBase().getDn().getUid());

                        logEintragService.handle(
                                mandant,
                                vzdObject.getDirectoryEntryBase().getTelematikID(),
                                emptyBusinessId ? "" : verzeichnisdienstImportCommand.getVerwaltungsId(),
                                vzdObject.getDirectoryEntryBase().getDn().getUid(),
                                null,
                ...
                if (verzeichnisdienstImportErgebnis.isInsert()) {
                    ...
                    DistinguishedName distinguishedName = (DistinguishedName) dnNameL.get(0);

                    verzeichnisdienstImportErgebnis.getLog().add("verzeichnisdienseintrag wurde eingefügt mit der id=" + distinguishedName.getUid() + " und der telematikid=" + verzeichnisdienstImportCommand.getTelematikID());
                    verzeichnisdienstImportErgebnis.setVzdUid(distinguishedName.getUid());

                    logEintragService.handle(
                            mandant,
                            verzeichnisdienstImportCommand.getTelematikID(),
                            emptyBusinessId ? "" : verzeichnisdienstImportCommand.getVerwaltungsId(),
                            distinguishedName.getUid(),
                            null,
                ...
                if (!verzeichnisdienstImportErgebnis.isDelete() && !verzeichnisdienstImportErgebnis.isInsert() && verzeichnisdienstImportCommand.toUpdate(verzeichnisdienstImportCommandContainer.getSektor(), vzdObject)) {
                    ...
                    verzeichnisdienstImportCommand.setVzdUid(vzdObject.getDirectoryEntryBase().getDn().getUid());
                    verzeichnisdienstImportCommand.setTelematikID(vzdObject.getDirectoryEntryBase().getTelematikID());
                    ...
                    verzeichnisdienstImportErgebnis.setTelematikID(vzdObject.getDirectoryEntryBase().getTelematikID());
                    verzeichnisdienstImportErgebnis.setVzdUid(vzdObject.getDirectoryEntryBase().getDn().getUid());
                    ...
                    if (!verzeichnisdienstImportCommand.getEncZertifikat().isEmpty()) {
                        List<UserCertificate> vzdCerts = verzeichnisdienstService.ladeZertifikate(tiVZDProperties, vzdObject.getDirectoryEntryBase().getDn().getUid());
